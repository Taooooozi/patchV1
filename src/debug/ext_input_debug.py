...
for sf in SF:
    for orient in D:
        for phase in P:
            video_fn = path + f'drifting_color_{n}'
            cfg_fn = path + f'drifting_color_{n}_cfg'
            ...
            f.close()
            generate_grating(1.0, sf, TF, orient,32, crest, valley, video_fn, time, phase, sharpness, frameRate = 96, ecc = range_deg, buffer_ecc = buffer_deg, gtype='drifting', neye = neye, bar = False, center = center, wing = wing, mask = None, inputLMS = inputLMS,genMovie = True)
            n = n+1
