...
int main(int argc, char** argv) {
    ...
    */
	...
	fstream fSnapshot;
	PosInt it0 = 0;
    vector<Float> sample_spikeCount;
    ...
    if (!minimal) { // output file tests
		...
		if (!fSample) {
            ...
            sample_spikeCount.assign(sampleSize, 0.0);
			LGN_spike_time.assign(nLGN, 0.0);
        }
    }
	...
	for (unsigned int it = 0; it < nt; it++) {
		...
		if (it > 0) { // seeking for overlap of d2h of V1 data  with LGN convol compute 
            if (minimal && it*dt >= sample_t0) { // send LGN_spike, V1_spike to minimal sample file
				for (PosInt j = 0.; j < sampleID.size(); j++) {
				    ...
				    if (sInfo >= 1) {
                        sample_spikeCount[j] += sInfo;
                    }
                }
				for (PosInt j = 0.; j < nLGN; j++) {
                    Float sInfo = LGN_sInfo[j];
					LGN_spike_time[j] = sInfo;
					// cout << LGN_spike_time[j] << '\n';
				}
        ...
        if ((minimal && it*dt >= sample_t0) || (!minimal && (learnData_FF > 1 || getLGN_sp))) { // send LGN_sInfo
        	...
        	if (getLGN_sp) targetAddress = LGN_sInfo;
			    ...
			    // cudaMemcpyAsync(targetAddress, currentConvol, nLGN*sizeof(Float), cudaMemcpyDeviceToHost, LGN_stream);
            #endif
    ...
    if (!minimal) {
		...
		fSample.write((char*)&(sample_spikeCount[0]), sampleSize*sizeof(Float));
        fSample.close();
    }
	{ // clean-up
		fStimulus.close();
